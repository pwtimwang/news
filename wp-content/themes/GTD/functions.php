...
function prologue_the_title($before = '<h2>', $after = '</h2>') {
	...
	error_log($pos);*/
$t1 = current_user_can( 'edit_post', get_the_id() ) ? 'editarea' : ' ';
$t2=get_the_ID();
$t3 = <<<test
...
<li id="prologue-<?php the_ID(); ?>" class="newupdates user_id_<?php the_author_ID( ); ?>">
	<h2 class="<?php if (current_user_can( 'edit_post', get_the_id() )) {?> editarea<?php } ?>" id="titlet-<?php the_ID(); ?>">
    ...
    <h4>
		...
		<span class="meta">
			...
			<span class="actions">
				<?php echo addsourcelink(); ?> 

				| <a href="<?php the_permalink( ); ?>" class="thepermalink"><?php _e('详细信息', 'p2'); ?></a>
	...
	<div class="postcontent<?php if (current_user_can( 'edit_post', get_the_id() )) {?> editarea<?php } ?>" id="content-<?php the_ID(); ?>"><?php the_content( __( '(More ...)' , 'p2') ); ?>
...
function prologue_navigation() { ?>
	...
	<?php if(!is_single()) { ?>
   		...
   		<?php posts_nav_link(' | ', __('&larr;&nbsp;最新发布的', 'p2'), __('较早发布的&nbsp;&rarr;', 'p2')); ?>

	<?php } else { ?>

		<?php previous_post_link('%link', __('&larr;&nbsp;上一篇', 'p2') ) ?> | <?php next_post_link('%link', __('下一篇&nbsp;&rarr;', 'p2')) ?>
...
function prologue_latest_posts_sidebar() {
...
<p class="weiboShow_mainFeed_listContent_txt">
	...
	<a title="<?php the_title(); ?>"><?php echo '【'.($post->post_title).'】'.(strip_tags($post->post_content)); ?></a>
...
function AjaxLoad(){
 ...
 if( isset($_GET['action'])){
            if($_GET['action'] == 'prologue_latest_posts_sidebar'  ){
			
		prologue_latest_posts_sidebar();
...
function suffusion_share($output)
{
	...
	$share_content = '【'.$post->post_title.'】'.$post->post_content.' via @PingWest中文网';
	$share_str = <<<test
<!-- JiaThis Button BEGIN -->
<div class="jiathis_style" id="ckepop" onmouseover="setShare('$share_content','$post->guid', '')" style="clear: both">
...
//{
	...
	//$excerpt_image=suffusion_get_image_from_custom_field('thumbnail');
	$appdix = '... @PingWest中文网';
	$orgial_content = '【'.$post->post_title.'】'.strip_tags($post->post_content) . $appdix;
	if(mb_strlen($orgial_content, 'UTF8') > 140 - 30)
	{
		$app_len =  mb_strlen($appdix , 'UTF8');
		$share_content = mb_substr($orgial_content, 0, 140 - 30 - $app_len , 'UTF8');
		$share_content = $share_content . $appdix;
	}
	else
	{
...
<!--
<a href="http://www.jiathis.com/send/?webid=copy&url=http://news.pingwest.com" class="jiathis_button_copy" title="复制网址">
<span class="jiathis_txt jtico jtico_copy"></span>
</a>
-->
...
function addsourcelink()
{
			...
			{
					$sourcelinkHTML = <<<test
			<a target="_blank" href="$sourcelink">来源</a> 
test;
				return $sourcelinkHTML;
			}
}
	

function delete_image($output)
{
	if(!is_single())
	{
...
add_filter('the_content', 'delete_image', 9);


//add_filter('the_content', 'addsourcelink', 10);

function outputCommentsNum()
{
	global $post;
	if(get_comments_number($post->ID) > 0)
	{
		echo "<a id='commentNum-".$post->ID."' href='".get_permalink($post->ID) . "' style='margin-left:20px'>【参与讨论（". get_comments_number($post->ID) . "）】</a>";
	}
	else
	{
