...
jQuery(function($) {
	...
	*/
	...
	function newPost(trigger) {
		...
		var posttext = GetTinyMceContent('content');

		var posttitile = $.trim($('#posttitile').val());

        var sourcelink = $.trim($('#sourcelink').val());
		...
		var args = {action: 'prologue_new_post', _ajax_post:nonce, posttext: posttext,posttitile:posttitile,tags: tags, sourcelink: sourcelink};
		var errorMessage = '';
		$.ajax({
			type: "POST",
			url: ajaxUrl,
			...
			case "post" :
				...
				if (inlineEditPosts != 0 && isUserLoggedIn) {
					...
					thisPostEditArea.editable(ajaxUrl, {event: 'edit',loadurl: ajaxUrl + '?action=prologue_load_post&_inline_edit=' + nonce,
					        ...
					        submitdata: {action:'prologue_inline_save', _inline_edit: nonce, post:'content'}});

					//post title
					thisPostTitleEditArea = $(element).children('h2.editarea');
					//thisPostTitleEditArea.editable(ajaxUrl);
//                                      $(element).children('.dblclick').editable(ajaxUrl,a
//                                           {
//                                           indicator: "<img srv='img/indicator.gif'>",
//                                           tooltip   : '',
//                                           event :"dblclick",
//                                           style:"inherit"});
                                        thisPostTitleEditArea.editable(ajaxUrl, {event: 'edit',loadurl: ajaxUrl + '?action=prologue_load_post&_inline_edit=' + nonce,
                                                loaddata:{get:'title'},
			                        id: 'post_ID', name: 'content', type: 'autogrow', cssclass: 'textedit',rows: '3',
			                        indicator : '<img src="' + templateDir +'/i/indicator.gif">', loadtext: p2txt.loading,
			                        cancel: p2txt.cancel, submit: p2txt.save, tooltip   : '', width: '90%', onblur: 'ignore',
			                        submitdata: {action:'prologue_inline_save', _inline_edit: nonce, post:'title'}});
                                     
					$(element).find('a.post-edit-link').click(function() {
						thisPostEditArea.trigger("edit");

						thisPostTitleEditArea.trigger("edit");
				...
				$(".single #postlist li > div.postcontent, .single #postlist li > h4, li[id^='prologue'] > div.postcontent, li[id^='comment'] > div.commentcontent, li[id^='prologue'] > h4, li[id^='comment'] > h4").hover(function() {
					$(this).parents("li").eq(0).addClass('selected');
					$(this).find(".jiathis_style").eq(0).fadeIn();
				}, function() {
					$(this).parents("li").eq(0).removeClass('selected');
					$(this).find(".jiathis_style").eq(0).fadeOut();
				});
				break
		}
	}
	...
	$(".single #postlist li > div.postcontent, .single #postlist li > h4, li[id^='prologue'] > div.postcontent, li[id^='comment'] > div.commentcontent, li[id^='prologue'] > h4, li[id^='comment'] > h4").hover(function() {
		...
		$(this).find("#sourcelink1").eq(0).css("color","#3478e3");
	}, function() {
		$(this).parents("li").eq(0).removeClass('selected');
		$(this).find(".jiathis_style").eq(0).fadeOut();

		$(this).find("#sourcelink1").eq(0).css("color","#ccc");
	});
	...
	if (inlineEditPosts && isUserLoggedIn) {
        ...
        //Tim added
        //post title
		thisPostTitleEditArea = $('h2.editarea');
        thisPostTitleEditArea.editable(ajaxUrl, {event: 'edit',loadurl: ajaxUrl + '?action=prologue_load_post&_inline_edit=' + nonce,
             loaddata:{get:'title'},
			 id: 'post_ID', name: 'content', type: 'autogrow', cssclass: 'textedit',rows: '3',
			 indicator : '<img src="' + templateDir +'/i/indicator.gif">', loadtext: p2txt.loading,
		...
		$('#main a.post-edit-link').click(function() {
            ...
            //Tim added
            $(this).parents('li').children('h2.editarea').trigger("edit");
            //Tim added
