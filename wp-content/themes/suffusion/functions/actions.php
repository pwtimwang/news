...
<?php
		...
		<div id="author-profile-<?php the_author_meta('user_nicename', $id); ?>" class="author-profile author-even fix" 
		                  style="padding-bottom:0px; height:110px; padding:0px">
			<?php echo suffusion_get_author_information_WPERI_1();?>
			<h2 class="author-title" style="border-bottom:none"><?php the_author_meta('display_name', $id); ?></h2>
			<?php echo suffusion_get_author_information_WPERI_2();?>
		</div><!-- /.author-profile -->
<?php
	}
...
//改变作者列表页作者介绍格局
function suffusion_get_author_information_WPERI_1() {
	$id = get_query_var('author');
    $ret = "<a class=\"authorImg\" style=\"float:left\" href=\"#\">";
	$ret .= get_avatar(get_the_author_meta('user_email', $id), '96')."\n";
	$ret .="</a>";
	$ret .= "<div class=\"author-description\" style=\"margin-left:138px\">\n";
return $ret;
}

//改变作者列表页作者介绍格局
function suffusion_get_author_information_WPERI_2() {
	$id = get_query_var('author');
	$ret .= "<p class=\"author-bio fix\">\n";
	$ret .= get_the_author_meta('description', $id)."\n";
	$ret .= "</p><!-- /.author-bio -->\n";
